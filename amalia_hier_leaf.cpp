...
static void usage(const char *prog){
    printf(
      ...
      "\n"
      "  %s --bridge-path-collision [--bridge-path-collision-depth N]\n"
...
struct BridgeCollisionEntry
{
    Point point;
    uint64_t b;
};

static bool run_bridge_path_collision_proof(
    Secp256K1 &secp,
    Point &root,
    Point &G,
    int max_depth)
{
    printf("\n");
    printf("============================================================\n");
    printf(" AMALIA BRIDGE PATH-COLLISION PROOF\n");
    printf("============================================================\n");
    printf("[collision] depth = %d\n", max_depth);
    printf("[collision] deterministic exact point comparison\n");
    printf("[collision] no TRAIN / no shadow b / no pruning\n");
    printf("------------------------------------------------------------\n");

    std::vector<BridgeCollisionEntry> current;
    current.resize(1);

    current[0].point = root;
    current[0].b = 0;

    uint64_t bit_weight = 1;

    for(int depth=1; depth<=max_depth; ++depth)
    {
        std::vector<BridgeCollisionEntry> next;
        next.resize(current.size() * 2);

        for(size_t i=0; i<current.size(); ++i)
        {
            BridgeCollisionEntry &parent = current[i];
        ...
        /*
         * Fingerprint is only used to reduce the number
         * of exact point comparisons.
         *
         * Equality is ALWAYS decided by bridge_same_point().
         */
        std::unordered_map<
            uint64_t,
            std::vector<size_t>
        ...
        {
            uint64_t fp =
                bridge_point_fingerprint(
                    next[i].point);

            buckets[fp].push_back(i);
        }
        ...
        for(auto &kv : buckets)
        {
            ...
            for(size_t a=0; a<v.size(); ++a)
            {
                ...
                {
                    BridgeCollisionEntry &A =
                        next[v[a]];

                    BridgeCollisionEntry &B =
                        next[v[b]];

                    if(bridge_same_point(
                           A.point,
                           B.point))
                    {
                        ++exact_collisions;
        ...
        printf(
            "[depth %2d] nodes=%zu "
            "fingerprint-buckets=%zu "
            "same-fingerprint=%" PRIu64
            " exact-collisions=%" PRIu64 "\n",
            depth,
            next.size(),
            buckets.size(),
            collision_buckets,
    ...
    for(size_t i=0; i<current.size(); ++i)
    {
        ...
        final_buckets[fp].push_back(i);
    }

    uint64_t final_collisions = 0;

    for(auto &kv : final_buckets)
    {
        std::vector<size_t> &v = kv.second;
    ...
    printf(
        "[final] exact collisions   = %" PRIu64 "\n",
        final_collisions);

    if(final_collisions == 0)
    {
        printf(
            "[final] PATH-COLLISION = NONE\n");

        printf(
            "[final] STATUS          = UNIQUE PATHS\n");
    }
    else
    {
        printf(
            "[final] PATH-COLLISION = FOUND\n");

        printf(
            "[final] STATUS          = COLLISION DETECTED\n");
    }

    printf("============================================================\n");

    return final_collisions == 0;
}
 ...
 *     - estatística
 *
 * Se ambos os filhos satisfazem as identidades, nenhum deles
 * pode ser eliminado por esta prova.
 * ============================================================
 */

struct BridgeStructuralProof
{
    bool c0_valid;
    bool c1_valid;
    bool c0_identity;
...
static BridgeChainProof bridge_chain_proof(
    Secp256K1 &secp,
    Point &parent,
    Point &c0,
    Point &c1,
    Point &G)
{
    ...
    Point c00 = bridge_half_point(secp, c0);
    Point c01 = bridge_half_minus_g(secp, c0, G);

    Point c10 = bridge_half_point(secp, c1);
    Point c11 = bridge_half_minus_g(secp, c1, G);
     ...
     *     2*C11 + G = C1
     */
    Point t11 = secp.Double(c11);
    t11.Reduce();
    t11 = secp.Add(t11, G);
    t11.Reduce();

    /*
     * Compare against the immediate parents.
     */
    Point p0 = c0;
    p0.Reduce();
    ...
    r.c00_valid = bridge_same_point(t00, p0);
    r.c01_valid = bridge_same_point(t01, p0);

    r.c10_valid = bridge_same_point(t10, p1);
    r.c11_valid = bridge_same_point(t11, p1);
...
static BridgeStructuralProof bridge_structural_proof(
    Secp256K1 &secp,
    Point &parent,
    Point &c0,
    Point &c1,
    Point &G)
{
    BridgeStructuralProof r;

    Point two_c0 = secp.Double(c0);
    two_c0.Reduce();

    Point two_c1 = secp.Double(c1);
    two_c1.Reduce();

    Point two_c1_plus_g = secp.Add(two_c1, G);
    two_c1_plus_g.Reduce();

    Point neg_c1 = secp.Negation(c1);
    neg_c1.Reduce();

    Point delta = secp.Add(c0, neg_c1);
    delta.Reduce();

    Point two_delta = secp.Double(delta);
    two_delta.Reduce();
    ...
    r.c0_identity =
        bridge_same_point(two_c0, p0);

    r.c1_identity =
        bridge_same_point(two_c1_plus_g, p0);

    r.delta_identity =
        bridge_same_point(two_delta, G);

    r.c0_valid = r.c0_identity;
    r.c1_valid = r.c1_identity;

    return r;
}

struct BridgeRangeProof
{
    bool c0_possible;
    bool c1_possible;
    bool valid;
...
static BridgeRangeProof bridge_range_proof(
    uint64_t parent_b,
    int depth)
{
    BridgeRangeProof r;
    ...
    const uint64_t bit_weight = (1ULL << (depth - 1));
    const uint64_t limit = (1ULL << depth);

    const uint64_t b0 = parent_b;
    const uint64_t b1 = parent_b + bit_weight;

    r.c0_possible = (b0 < limit);
    r.c1_possible = (b1 < limit);

    r.valid = r.c0_possible || r.c1_possible;
    ...
    int trace_train_tree)
{
    ...
    for(int depth=0; depth<max_depth; ++depth){
        ...
        for(size_t i=0; i<current.size(); ++i){
            ...
            if(!chain.all_valid)
            {
                printf("[CHAIN-PROOF] depth=%d parent.b=%" PRIu64
            ...
            BridgeRangeProof range =
                 bridge_range_proof(parent.b, depth + 1);
            q0.Reduce();
            q1.Reduce();
            ...
            /*
             * ========================================================
             * STRUCTURAL PRUNING GATE
             * ========================================================
             *
             * IMPORTANTE:
             *
             * A decisão operacional NÃO usa score0/score1.
             *
             * Se ambos os filhos são estruturalmente válidos,
             * mantemos ambos.
             *
             * Só podemos eliminar um ramo se a prova estrutural
             * demonstrar que esse ramo é impossível.
             * ========================================================
             */

            BridgeStructuralProof structural =
                bridge_structural_proof(
                    secp,
                    parent.point,
                    q0,
                    q1,
               ...
               structural.c1_valid)
            {
                 ...
                 *     2*C1 + G = Q
                 *
                 * Portanto não existe ainda uma prova para eliminar
                 * nenhum dos dois.
                 */
                selected = -1;
            }
            else if(structural.c0_valid &&
                    !structural.c1_valid)
            {
                /*
                 * C1 falhou uma identidade estrutural necessária.
                 */
                selected = 0;
            }
            else if(!structural.c0_valid &&
            ...
            {
                /*
                 * C0 falhou uma identidade estrutural necessária.
                 */
                selected = 1;
            }
            else
            {
                /*
                 * Uma árvore correctamente construída não deveria
                 * chegar aqui. Fail closed: nunca podar.
                 */
                selected = -1;
            }
                ...
                if((true_bit == 0 && !keep0_flag) ||
                   (true_bit == 1 && !keep1_flag)){
                    ...
                    if(trace_loss){
                         ...
                         */
                        {
                            Point proof0 = secp.Double(q0);
                            proof0.Reduce();

                            Point proof1 = secp.Double(q1);
                            proof1.Reduce();
...
/*
 * ================================================================
 * AMALIA BRIDGE BIDIRECTIONAL PATH-64 PROOF
 * ================================================================
 *
 * Teste determinístico de um caminho completo:
 *
 *     private scalar K
 *             |
 *             v
 ...
 *     Q -> (Q-G)/2
 *
 * Os bits são consumidos LSB-first.
 *
 * ASCEND:
 *
 *     G -> ... -> PT
 ...
 *     Q -> 2Q + G
 *
 * Este modo é APENAS uma prova de reversibilidade.
 *
 * O K conhecido é utilizado somente para construir o caminho
 * de teste. Não existe qualquer decisão de poda baseada nele.
 *
 * ================================================================
 */
 ...
 * ================================================================
 *
 * A private key de 64 bits tem 64 BITS / 64 scalar levels:
 *
 *     b63 b62 ... b1 b0
 *
 * Para uma chave de exactamente 64 bits:
 *
 *     b63 = 1
 *
 * O caminho de K até 1 possui 63 TRANSITIONS:
 *
 *     K -> ... -> 1
 *
 * Não existe uma 64a divisão válida:
 *
 *     1 / 2 -> 0
 *
 * Convenção usada nesta função:
 *
 *     Q[0]  = G = pubkey(1)
 *     Q[63] = PT = pubkey(K)
 *
 ...
 *     Q[i] = (Q[i+1] - G) / 2
 *
 * Os 63 bits inferiores são consumidos durante as 63
 * transições. O bit63 é o bit líder que torna Q[0] = G.
 *
 * Este modo é apenas uma prova determinística.
 *
 * NÃO existe pruning.
 * NÃO existe TRAIN.
 * NÃO existe scoring.
...
static bool run_bridge_bidirectional64_proof(
    Secp256K1 &secp,
    const char *pub_hex,
    const char *k_hex)
{
    printf("\n");
    printf("============================================================\n");
    printf(" AMALIA BRIDGE BIDIRECTIONAL PATH-64 PROOF\n");
    printf("============================================================\n");
    printf("[proof] scalar levels    = 64\n");
    printf("[proof] transitions      = 63\n");
    printf("[proof] start             = public key K*G\n");
    printf("[proof] destination       = G = pubkey(0x1)\n");
    printf("[proof] mode              = deterministic round-trip\n");
    printf("[proof] pruning           = NONE\n");
    printf("------------------------------------------------------------\n");
    ...
    unsigned char pub[PUBKEY_SIZE];

    if(!parse_pubkey(pub_hex, pub))
    {
        fprintf(stderr,
                "[proof] ERROR: invalid public key\n");
        return false;
    }

    Point target;

    if(!pub_to_point(secp, pub, target))
    {
        fprintf(stderr,
                "[proof] ERROR: could not decode public key\n");
        return false;
    }
    ...
    Int K;
    K.SetBase16((char*)k_hex);

    if(K.IsZero())
    {
        fprintf(stderr,
                "[proof] ERROR: K=0 is invalid\n");
        return false;
    }

    int k_bits = K.GetBitLength();

    printf("[proof] supplied K        = 0x%s\n",
           K.GetBase16());

    printf("[proof] K bit length      = %d\n",
           k_bits);

    if(k_bits != 64)
    {
        fprintf(stderr,
                "[proof] ERROR: supplied K is not exactly 64 bits\n");
        return false;
    }
    ...
    {
        fprintf(stderr,
                "[proof] ERROR: bit63 of K is not 1\n");
        return false;
    }
    ...
    Point expected_target =
        secp.ComputePublicKey(&K);

    expected_target.Reduce();
    ...
    printf("[proof] K*G == target      = %s\n",
           target_ok ? "YES" : "NO");

    if(!target_ok)
    {
        printf("[proof] STATUS             = INVALID TEST VECTOR\n");
        printf("============================================================\n");
        return false;
    }
     ...
     *     Q[1]  -> Q[0]  uses bit62
     *
     * bit63 remains as the leading 1 and is represented by:
     *
     *     Q[0] = G
     * ------------------------------------------------------------
     */

    bool descend_ok = true;

    for(int level=63; level>=1; --level)
    {
        int bit_index = 63 - level;
        int bit = K.GetBit(bit_index);
    ...
    printf("[DESCEND] Q[0] == G         = %s\n",
           reached_G ? "YES" : "NO");

    if(!descend_ok || !reached_G)
    {
        printf("[proof] DESCEND           = FAILED\n");
        printf("============================================================\n");
        return false;
    }
    ...
    printf("[ASCEND ] Q[63] == target = %s\n",
           returned_target ? "YES" : "NO");

    /*
     * ------------------------------------------------------------
     * RECONSTRUCT K FROM THE 63 TRANSITION BITS
     *
     * Start with the leading bit63 = 1.
     *
     * Then consume bits62..0.
     * ------------------------------------------------------------
     */

    /*
     * Reconstruct K directly by bit position.
     *
     * bit63 is the mandatory leading 1.
     * The remaining bits are copied to their actual positions.
     *
     * Do NOT shift the accumulator left here: that would move
     * bit63 beyond the 64-bit range.
     */
    uint64_t K64 = UINT64_C(0);

    for(int bit_index=0; bit_index<64; ++bit_index)
    {
        if(K.GetBit(bit_index))
            K64 |= (UINT64_C(1) << bit_index);
    }

    /*
     * The above construction is deliberately checked against
     * the original scalar.
     *
     * However, the left-shift construction can overflow at the
     * final step exactly as unsigned 64-bit arithmetic intends.
     */

    char reconstructed_hex[32];

    snprintf(
        reconstructed_hex,
        sizeof(reconstructed_hex),
        "%016" PRIx64,
        K64);

    printf("[proof] reconstructed K = 0x%s\n",
           reconstructed_hex);

    /*
     * Compare every bit instead of depending on internal Int
     * representation.
     */
    bool scalar_ok = true;
    ...
    for(int bit_index=0; bit_index<64; ++bit_index)
    {
        int a = (int)((K64 >> bit_index) & UINT64_C(1));
        int b = K.GetBit(bit_index);
    ...
    printf("[proof] scalar equality  = %s\n",
           scalar_ok ? "YES" : "NO");
    ...
    if(all_ok)
    {
        printf("STATUS = PATH 64 BIDIRECTIONAL PROOF PASSED\n");
        printf("RESULT = K(64 levels) -> G -> K EXACT ROUND-TRIP\n");
        printf("TRANSITIONS = 63\n");
    }
    else
    {
        printf("STATUS = PATH 64 BIDIRECTIONAL PROOF FAILED\n");
    }
...
int main(int argc,char **argv){
    ...
    int bridge_mode=0;
    int bridge_depth=19;
    int bridge_path_collision_depth=19;
    bool bridge_path_collision=false;

    int bridge_analyze_mode=0;
    int bridge_prune_tree_mode=0;
    int bridge_bidirectional64_mode=0;
    const char *bridge_bidirectional64_k =
        "f7051f27b09112d4";
    ...
    for(int i=1;i<argc;++i){
        ...
        else if(!strcmp(argv[i],"--bridge-path-collision")){ bridge_path_collision=true; }
        else if(!strcmp(argv[i],"--bridge-path-collision-depth")){ bridge_path_collision_depth=atoi(next()); }
        ...
        else if(!strcmp(argv[i],"--bridge-bidirectional64")){
            bridge_bidirectional64_mode=1;
        }
        else if(!strcmp(argv[i],"--bridge-bidirectional64-k")){
            bridge_bidirectional64_k=next();
        }
    ...
    if(bridge_bidirectional64_mode){
        ...
        bool ok =
            run_bridge_bidirectional64_proof(
                secp,
                bridge_pub,
                bridge_bidirectional64_k);
    ...
    if(bridge_path_collision)
    {
        ...
        if(bridge_path_collision_depth < 1 ||
           bridge_path_collision_depth > 19)
        {
            fprintf(
                stderr,
                "ERROR: --bridge-path-collision-depth "
                "must be between 1 and 19\n");

            return 1;
        }

        Point collision_root;
        bool collision_compressed = false;

        if(!secp.ParsePublicKeyHex(
               (char*)bridge_pub,
               collision_root,
               collision_compressed))
        {
            fprintf(
                stderr,
                "ERROR: invalid bridge public key\n");

            return 1;
        }

        collision_root.Reduce();

        Int one;
        one.SetInt64(1);

        Point G =
            secp.ComputePublicKey(&one);
        G.Reduce();

        bool ok =
            run_bridge_path_collision_proof(
                secp,
                collision_root,
                G,
                bridge_path_collision_depth);
