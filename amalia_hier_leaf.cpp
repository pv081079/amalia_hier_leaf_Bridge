    ...
    std::vector<TreeRule> &rules)
{
    ...
    char line[256];
    int depth_count=0;
    if(!fgets(line,sizeof(line),fp) || sscanf(line,"depth %d",&depth_count)!=1){
    ...
    printf("[tree] loaded V2 rule file = %s\n",path);
    return true;
}
    ...
    const char *rule_file)
{
    ...
    /* Persist the complete selected rule table.  No training sample K
     * and no shadow b are serialized. */
