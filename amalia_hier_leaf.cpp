    ...
    const std::vector<int> &features)
{
    ...
    if(!fp){
        perror("[tree] fopen rule file");
        return false;
    }

    fprintf(fp, "AMALIA_BRIDGE_RULE_V1\n");
    fprintf(fp, "depth %zu\n", features.size());
    ...
    std::vector<int> &features)
{
    ...
    if(strncmp(header, "AMALIA_BRIDGE_RULE_V1", 21) != 0){
        fclose(fp);
        fprintf(stderr,
                "[tree] invalid rule file header: %s\n",
                path);
        return false;
    }
    ...
    if(sscanf(line, "depth %d", &depth_count) != 1){
        fclose(fp);
        return false;
    }
    ...
    if(depth_count != max_depth){
        fclose(fp);
        fprintf(stderr,
                "[tree] rule depth mismatch: file=%d requested=%d\n",
                depth_count,
                max_depth);
        return false;
    }

    features.assign((size_t)max_depth, -1);
    ...
    for(int i=0; i<depth_count; ++i){
        ...
        if(sscanf(line, "%u %d", &depth, &feature) != 2){
            fclose(fp);
            return false;
        }

        if(depth >= (unsigned int)max_depth){
            fclose(fp);
            return false;
        }

        features[(size_t)depth] = feature;
    }

    fclose(fp);

    for(int d=0; d<max_depth; ++d){
        if(features[(size_t)d] < -1 ||
           features[(size_t)d] >= BRIDGE_ANALYZE_FEATURES){
            fprintf(stderr,
                    "[tree] invalid feature at depth %d: %d\n",
                    d,
                    features[(size_t)d]);
            return false;
        }
    ...
    printf("[tree] loaded rule file = %s\n", path);
    ...
    const char *rule_file)
{
    ...
    std::vector<int> frozen_features;
    ...
    struct TreeRule {
        ...
        TreeRule()
            : feature(-1), unique_values(0), train_obs(0) {}
    };
    ...
    for(int depth=0; depth<max_depth; ++depth){
        ...
        if(loaded_frozen_rule){
            TreeRule frozen_rule;

            frozen_rule.feature =
                frozen_features[(size_t)depth];

            rules[(size_t)depth] =
                std::move(frozen_rule);
    ...
    /*
     * If training was performed, persist only the selected feature
     * for each depth.  The statistical lookup table remains in
     * memory for this run; the persisted file deliberately contains
     * no training samples and no shadow b.
     */
    if(rule_file && *rule_file && !loaded_frozen_rule){
        ...
        for(int d=0; d<max_depth; ++d)
            selected_features[(size_t)d] =
                rules[(size_t)d].feature;

        if(!bridge_save_tree_rules(
               rule_file,
               selected_features)){
            fprintf(stderr,
                    "[tree] WARNING: could not save rule file\n");
        }
    }
    ...
    if(loaded_frozen_rule){
        fprintf(stderr,
                "[tree] ERROR: frozen feature-only rule is not sufficient "
                "for branch decisions in this version.\n");
        fprintf(stderr,
                "[tree] Use the training mode to generate the complete "
                "rule table before enabling frozen execution.\n");
        return false;
    }
    ...
    for(int depth=0; depth<max_depth; ++depth){
        ...
        for(size_t i=0; i<current.size(); ++i){
            ...
            if(rule.feature >= 0){
                ...
                if(score0 > score1) selected = 0;
                else if(score1 > score0) selected = 1;
            }
