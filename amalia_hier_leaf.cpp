...
/*
 * ================================================================
 * PAIRED CHILD RANKING ANALYSIS
 * ================================================================
 */

printf("\n");
printf("======================================================================\n");
printf(" PAIRED CHILD RANKING - OUT OF SAMPLE\n");
printf("======================================================================\n");

printf("\n");
printf("[paired] child0 = Q/2\n");
printf("[paired] child1 = (Q-G)/2\n");
printf("[paired] delta   = feature(child1) - feature(child0)\n");
printf("[paired] orientation learned from TRAIN only\n");
printf("[paired] TEST is never used to select orientation\n");
printf("\n");

printf("======================================================================\n");
printf(" depth | feature                    | orient | test acc | decisive | mean margin\n");
printf("----------------------------------------------------------------------\n");
...
for(int depth = 0;
    ...
    for(int f = 0;
         ...
         * ------------------------------------------------------------
         */
        BridgePairEval test_eval =
            bridge_eval_paired_feature(
                obs,
                train_samples,
                test_samples,
                orientation
        ...
        double accuracy =
            bridge_pair_accuracy(
                test_eval
            );

        double decisive =
            bridge_pair_decisive_rate(
                test_eval
            );

        printf(
            " depth=%2d  %-24s  %6s  %8.4f%%  %8.4f%%  %+11.4f\n",
            depth + 1,
            bridge_feature_name(f),
            orientation > 0
                ? "+1"
...
/*
 * ================================================================
 * BEST PAIRED FEATURE PER DEPTH
 * ================================================================
 */

printf("\n");
printf("============================================================\n");
printf(" BEST PAIRED FEATURE PER DEPTH\n");
printf("============================================================\n");
...
for(int depth = 0;
    ...
    double best_acc = 50.0;
    double best_decisive = 0.0;
    ...
    for(int f = 0;
        ...
        /*
         * Não queremos simplesmente maximizar accuracy
         * de features que quase nunca são decisivas.
         *
         * Primeiro exigimos que exista pelo menos uma
         * fracção razoável de comparações decisivas.
         *
         * O score principal continua a ser a distância
         * a 50%.
         */
        double score =
            fabs(accuracy - 50.0);
        ...
        if(score > best_score){
            ...
            best_margin = test_eval.mean_margin;
    ...
    if(best_feature < 0){
        ...
        printf(
            "depth=%2d  %-24s  no usable paired signal\n",
            depth + 1,
            "none"
        );
        ...
        printf(
            "depth=%2d  %-24s  acc=%8.4f%%  "
            "delta=%+8.4f%%  decisive=%8.4f%%  "
            "margin=%+11.4f  orient=%+d\n",
...
printf("\n");
printf("============================================================\n");
printf(" PAIRED RANKING INTERPRETATION\n");
printf("============================================================\n");
printf("50%%       -> no measurable ranking signal\n");
printf(">50%%      -> possible paired signal\n");
printf("decisive  -> fraction of pairs with non-zero delta\n");
printf("margin    -> signed strength of the ranking\n");
printf("\n");
printf("[paired] This test DOES NOT prune branches.\n");
printf("[paired] Orientation is learned from TRAIN only.\n");
printf("[paired] TEST is strictly out-of-sample.\n");
printf("============================================================\n");
 ...
 * e calculamos:
 *
 *      delta = F(child1) - F(child0)
 *
 * Para cada sample sabemos qual dos dois era o verdadeiro.
 *
 * TRAIN:
 *
 *      determina apenas a orientação do delta.
 *
 * TEST:
 *
 *      orientação congelada.
 *
 * Não existe pruning neste teste.
 *
 * O objectivo é saber se existe uma propriedade RELATIVA
 * entre os dois filhos que permita ordená-los melhor que
 * uma escolha aleatória.
 * ================================================================
 */

struct BridgePairEval {
    uint64_t pairs;
...
/*
 * Calcula a orientação usando APENAS o TRAIN.
 *
 * Para cada sample:
 *
 *      d = value(child1) - value(child0)
 *
 * Se branch 1 for verdadeiro:
 *
 *      queremos +d
 *
 * Se branch 0 for verdadeiro:
 *
 *      queremos -d
 *
 * Portanto:
 ...
 *      signed_margin = (+/-1) * d
 *
 * Se a média for positiva:
 *
 *      delta > 0 favorece branch 1
 *
 ...
 *
 *      +1  -> orientação normal
 *      -1  -> orientação invertida
 *       0  -> sem sinal no TRAIN
 */
static int bridge_pair_orientation(
    const std::vector<BridgeFeatureObs> &obs,
    uint64_t train_samples)
{
    ...
    for(uint64_t s = 0;
        ...
        /*
         * O armazenamento é sempre:
         *
         *   base     = child0
         *   base + 1 = child1
         */
        const BridgeFeatureObs &c0 = obs[base];
        const BridgeFeatureObs &c1 = obs[base + 1];

        int64_t delta;

        /*
         * Diferença em signed 64-bit.
         *
         * A subtracção é feita em unsigned para evitar
         * comportamento indefinido e depois interpretada
         * como diferença modular.
         *
         * Para as features actuais isto é suficiente.
         */
        uint64_t ud = c1.value - c0.value;

        /*
         * Converter para signed.
         *
         * Não usamos directamente int64_t(ud) para evitar
         * qualquer ambiguidade de implementação.
         */
        if(ud <= UINT64_C(0x7fffffffffffffff))
            delta = (int64_t)ud;
        else
            delta = -(int64_t)(UINT64_MAX - ud) - 1;

        /*
         * Qual é o branch verdadeiro?
         *
         * branch 0 -> queremos -delta
         * branch 1 -> queremos +delta
         */
        if(c0.branch == 0)
            sum -= (long double)delta;
        else
            sum += (long double)delta;
...
/*
 * Avaliação OUT-OF-SAMPLE do ranking emparelhado.
 *
 * A orientação foi aprendida no TRAIN.
 *
 * Para cada par:
 *
 *      delta = F1 - F0
 ...
 * Se orientation == +1:
 *
 *      delta > 0 -> escolher child1
 *      delta < 0 -> escolher child0
 *
 * Se orientation == -1:
 *
 *      delta > 0 -> escolher child0
 ...
 * Em caso de delta == 0:
 *
 *      empate, sem informação.
 */
static BridgePairEval bridge_eval_paired_feature(
    const std::vector<BridgeFeatureObs> &obs,
    uint64_t first_sample,
    uint64_t sample_count,
    int orientation)
{
    BridgePairEval result;

    if(orientation == 0)
        return result;

    long double margin_sum = 0.0L;
    long double abs_margin_sum = 0.0L;
    ...
    for(uint64_t s = 0;
        ...
        const BridgeFeatureObs &c0 =
            obs[base];

        const BridgeFeatureObs &c1 =
            obs[base + 1];
        ...
        if(ud <= UINT64_C(0x7fffffffffffffff))
            ...
            delta =
                -(int64_t)(UINT64_MAX - ud) - 1;

        /*
         * signed_delta representa:
         *
         *      positivo -> favorece child1
         *      negativo -> favorece child0
         */
        ...
        if(true_branch == 1)
            margin = (long double)signed_delta;
        else
            margin = -(long double)signed_delta;
    ...
    if(result.pairs > 0){
        result.mean_margin =
            (double)(
                margin_sum /
                (long double)result.pairs
            );

        result.mean_abs_margin =
            (double)(
                abs_margin_sum /
                (long double)result.pairs
            );
...
static double bridge_pair_accuracy(
    const BridgePairEval &e)
{
    if(e.decisive == 0)
        return 50.0;
...
static double bridge_pair_decisive_rate(
    const BridgePairEval &e)
{
    if(e.pairs == 0)
        return 0.0;
