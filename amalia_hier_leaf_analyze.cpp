    ...
    uint64_t seed)
{
    ...
    for(uint64_t sample=0; sample<samples; ++sample){
        ...
        Point Q = secp.ComputePublicKey(&K);
        Q.Reduce();

        for(int depth=0; depth<max_depth; ++depth){

            /*
             * O bit correcto é obtido directamente do K conhecido
             * da amostra.
             */
            int true_bit =
                (int)((K64 >> depth) & UINT64_C(1));

            Point q0 = bridge_half_point(secp, Q);
            Point q1 = bridge_half_minus_g(secp, Q, G);
            ...
            for(int f=0; f<BRIDGE_ANALYZE_FEATURES; ++f){
                ...
                observations[(size_t)depth][f].push_back(o);
            }
             ...
             * Isto é crucial:
             *
             * não estamos a construir uma árvore 2^depth.
             *
             * Temos apenas uma cadeia por amostra.
             */
            Q = true_bit ? q1 : q0;
            Q.Reduce();
        }
        ...
        if(((sample + 1) % 1000) == 0 ||
           sample + 1 == samples){
            ...
            printf("\r[analyze] generated %" PRIu64
                   "/%" PRIu64 " samples",
                   sample + 1,
                   samples);
    ...
    printf("============================================================\n");
    printf("~50%%       -> no measurable predictive signal\n");
    printf(">50%%       -> possible signal; must be replicated\n");
    printf("stable >50 -> interesting candidate for deeper study\n");
    printf("\n");
    printf("[analyze] This experiment DOES NOT prune branches.\n");
    printf("[analyze] No feature is assumed to reveal a secret bit.\n");
    printf("============================================================\n");
